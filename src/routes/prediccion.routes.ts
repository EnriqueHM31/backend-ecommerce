...
interface RequestPrediccion extends Request {
    body: {
        usuario: string;
        compras?: Compra[];
        entrenar?: boolean;
...
interface RequestEntrenamiento extends Request {
    body: {
        compras: Compra[];
    };
}

interface DatosPreprocessed {
    matriz: number[][];
    numUsuarios: number;
    numProductos: number;
}
...
PrediccionRouter.post('/prediccion', async (req: RequestPrediccion, res: Response) => {
    try {
        ...
        // Si se solicita entrenamiento o es la primera vez
        if (entrenar || !sistemaRecomendacion.isInitialized) {
            if (!compras || !Array.isArray(compras) || compras.length === 0) {
                res.status(400).json({
                    error: 'Los datos de "compras" son requeridos para entrenar el modelo'
                });
                return
            }

            // Validar formato de compras
            ...
            // ✅ Entrenar modelo
            //await sistemaRecomendacion.entrenar(compras ?? []);
        }
...
PrediccionRouter.post('/prediccion/entrenar', async (req: RequestEntrenamiento, res: Response) => {
    try {
        ...
        if (!compras || !Array.isArray(compras) || compras.length === 0) {
            res.status(400).json({
                error: 'Los datos de "compras" son requeridos'
            });
        }
